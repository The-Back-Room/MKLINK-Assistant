...
IF "%TYPE%"=="SYMLINK" (
   ...
   RMDIR /S /Q "%TARGETPATH%" 1>NUL
   MKLINK /D "%LINKPATH%" "%TARGETPATH%" 1>NUL
   ATTRIB +H "%TARGETPATH%" 1>NUL
) ELSE IF "%TYPE%"=="HARDLINK" (
   MKLINK /H "%LINKPATH%" "%TARGETPATH%" 1>NUL
   ATTRIB +H "%TARGETPATH%" 1>NUL
) ELSE IF "%TYPE%"=="JUNCTION" (
   COPY "%TARGETPATH%" "%LINKPATH%"
   DEL /F /S /Q "%TARGETPATH%" 1>NUL
   RMDIR /S /Q "%TARGETPATH%" 1>NUL
   MKLINK /J "%LINKPATH%" "%TARGETPATH%" 1>NUL
   ATTRIB +H "%TARGETPATH%" 1>NUL
) ELSE (
    GOTO ERROR
)
